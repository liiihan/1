...
if __name__ == "__main__":
    ...
    image_name = base_path + '/generate/face/outIMG/' + input_name + '.jpg'
    ...
    fuzzyRandom, fuzzyPublic = extractor.generate(arr)

    with open(base_path + '/generate/face/descriptor.txt', 'w') as fo:
        fo.write(Hex_Distance)
    with open(base_path + '/generate/face/fuzzyRandom.txt', 'wb') as fo:
        fo.write(fuzzyRandom)
    print('face_Random1: {}'.format(str(fuzzyRandom)))

    np.savetxt(base_path + '/SecretSharing/fuzzyPublic/face/fuzzyCiphers.txt', fuzzyPublic[0], fmt='%d' ,delimiter=' ')
    np.savetxt(base_path + '/SecretSharing/fuzzyPublic/face/fuzzyMasks.txt', fuzzyPublic[1], fmt='%d',delimiter=' ')
    np.savetxt(base_path + '/SecretSharing/fuzzyPublic/face/fuzzyNonces.txt', fuzzyPublic[2], fmt='%d',delimiter=' ') 